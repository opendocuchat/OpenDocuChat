...
import { BotMessageSquare, ChevronDown, ChevronUp, Loader2 } from "lucide-react"
...
export default function ChatWidgetPage() {
  ...
  const createColorMapping = (docs: Document[]): ColorMap => {
    const colors = [
      {
        ...
        hover: "hover:bg-cyan-400 dark:hover:bg-cyan-700 hover:saturate-150 hover:contrast-125",
      },
      {
        base: "bg-lime-200 dark:bg-lime-800",
        hover: "hover:bg-lime-300 dark:hover:bg-lime-700 hover:saturate-150 hover:contrast-125",
      },
      {
        base: "bg-amber-200 dark:bg-amber-800",
        hover: "hover:bg-amber-300 dark:hover:bg-amber-700 hover:saturate-150 hover:contrast-125",
      },
    ]
    const defaultColor = {
    ...
    docs.forEach((doc, index) => {
      if (index < 3) {
        ...
        newColorMapping[doc.id] = index % 2 === 0 ? defaultColor : alternateColor
      }
    })
    return newColorMapping
  }

  const highlightText = (text: string, messageCitations: Citation[], colorMap: ColorMap) => {
    ...
    sortedCitations.forEach((citation, index) => {
      ...
      const segmentIndex = segments.findIndex((segment) => !segment.isHighlight && segment.text.includes(citation.text))
  ...
  useEffect(() => {
    ...
    const highlightRelatedElements = (documentId: string) => {
      const relatedElements = document.querySelectorAll(`[data-document-id="${documentId}"]`)
  ...
  const formatLLMResponse = (text: string, messageCitations: Citation[], colorMap: ColorMap) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await fetch(
        `/api/chat?message=${encodeURIComponent(inputMessage)}&conversationId=${conversationId}`,
        {
          method: "POST",
        }
      )
  ...
  const AdvancedHealthBar = ({ score, method = "logistic" }: { score: number; method?: string }) => {
    ...
    const scaledScore = scaleMethods[method as keyof typeof scaleMethods](clampedScore)

    return (
      <div className="w-full h-4 bg-gray-300 rounded-full overflow-hidden">
        <div
          className="h-full bg-gradient-to-r from-orange-300 to-[#ec573f] transition-all"
    ...
    <div className="w-screen h-screen flex flex-col bg-white dark:bg-gray-800 rounded-lg">
      <div className="flex-shrink-0 flex justify-between items-center p-4 border-b dark:border-slate-700">
        ...
        <div className="flex items-center">
          <Button variant="ghost" size="icon" className="mr-3 px-2 w-full" onClick={resetChat} title="New chat">
            {/* <RefreshCw className="h-4 w-4" /> */}
            New Chat
          </Button>
          <Button className="font-bold" variant="ghost" size="sm" onClick={() => setIsOpen(false)}>
            X
          </Button>
        </div>
      </div>
      ...
      <div className="flex-grow overflow-y-auto p-4">
          ...
          <div key={index} className={`mb-4 ${msg.role === "user" ? "text-right" : "text-left"}`}>
            <span
              className={`inline-block p-2 rounded-lg ${
                msg.role === "user" ? "bg-[#ec573f] text-white" : "bg-slate-200 dark:bg-gray-700"
              }`}
            >
        ...
        {documents.length > 0 && (
          <div className="mt-4">
            <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-2">Sources</h3>
            <ul className="text-xs space-y-2">
              {documents.slice(0, showAllSources ? undefined : 3).map((doc, index) => (
                <li
                  key={doc.id}
                  className={`document-item p-2 rounded flex justify-between ${colorMapping[doc.id].base} ${
